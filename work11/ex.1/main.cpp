...
#include <fstream>
#include <ctime>
#include "file.cpp" // Включаем файл с функциями
using namespace std;
...
// ввод размера с проверкой
    ...
    if (choice == 1) {
        ...
        if (!fin.is_open()) { // используем isopen() для проверки
    ...
    // вызов функции для перестановки минимального и максимального элементов
    SwapMinMaxElements(M, n, m);
    // вывод матрицы после перестановки на экран
    cout << "\nМатрица после перестановки минимального и максимального элементов:" << endl;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cout << M[i][j] << "\t";
        }
